...
import React, { useState } from 'react';
import axios from 'axios';
import { Grid, TextField, Button, Checkbox, Typography, FormLabel, RadioGroup, FormControlLabel, Input } from '@mui/material';
import { useMediaQuery } from '@material-ui/core';
import Radio from '@mui/material/Radio';
...
export const Registro = () => {
  ...
  const isValidPassword = (password) => {
    ...
    const passwordPWD_REGEX = /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{8,}$/;
  ...
  const handleRegister = async (event) => {
    ...
    if (!selectedImage) {
      Swal.fire({
        ...
        background: 'rgba(255,255,255,0.5)',
        showConfirmButton: false,
        title: `<h2 style="font-size:1rem; font-family: 'Oswald', sans-serif">Debes adjuntar el N.I.F</h2>`,
        timer: 4500,
        toast: true,
        color: 'black',
      });
      return; // Detener el flujo de la función si no se adjunta una imagen
    }
    ...
    if (!isValidPassword(password)) {
      Swal.fire({
        ...
        background: 'rgba(255,255,255,0.5)',
        showConfirmButton: false,
        title: `<h2 style="font-size:1rem; font-family: 'Oswald', sans-serif">La contraseña no es válida</h2>`,
        timer: 4500,
        toast: true,
        color: 'black',
      });
      return; // Detener el flujo de la función si la contraseña no es válida
    }
        ...
        >
          <Grid item xs={6} sm={6}>
            <TextField
              ...
              className="dark-input"
              onChange={(event) => {
                const alphabeticValue = event.target.value.replace(/[^a-zA-Z\sÑñ]/g, '').toUpperCase();
            ...
            <Grid item xs={12} sm={12}>
              <label htmlFor="upload-button">
                ...
                <Button
                  ...
                  sx={{
                    marginTop: "10px",
                    bgcolor: imageUploaded ? "#4CAF50" : "#CCCCCC", height: "18px",
                    color: "black",
                    "&:hover": {
                      bgcolor: "#6e6e6e",
                      color: "white"
          ...
          <Typography sx={{ marginLeft: matches ? '6.5em' : '2.5em', marginTop: '5px', color: 'white' }}>
            ELIGE TU PERFIL
          </Typography>
          <FormLabel sx={{ display: 'flex' }}></FormLabel>
          <RadioGroup required
          ...
          <Grid sx={{ display: 'flex', flexDirection: 'row-reverse', margin: 'auto' }}>
            <Typography onClick={showAlert} variant="span" sx={{ color: 'white', marginTop: matches ? '10px' : '10px', cursor: 'pointer' }}>
